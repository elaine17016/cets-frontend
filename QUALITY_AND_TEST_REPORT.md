...
| 整體覆蓋率（本地 Vitest） | **54.83%**（由 27.3% 大幅提升） |
| 安全性評級 | **A**（0 漏洞） |
| 可靠性評級 | **A**（0 Bug） |
| 可維護性評級 | **A** |
| 安全熱點審查 | **100%** |
...
| Coverage（整體覆蓋率） | **54.83%**（本地 Vitest） | — | 較先前 27.3% 提升約 **2 倍** |
| Duplications（重複程式碼） | 0.0% | — | 無重複程式碼區塊 |

> **關於覆蓋率的說明：**  
> SonarCloud 的 **新程式碼覆蓋率** 衡量本次提交新增/修改的程式碼是否被測試覆蓋，這是 Quality Gate 的判定依據。  
> **整體覆蓋率** 涵蓋整個程式碼庫。本次大幅補強後，本地 Vitest 整體覆蓋率由 **27.3% → 54.83%**；SonarCloud 儀表板會在 CI 完成後同步更新。
...
| # | 測試名稱 | 驗證內容 |
|---|----------|----------|
| 45 | exposes expected status labels | 活動/場次/報名/票券/角色/通知狀態中文標籤 |
...
### 7.1 整體測試覆蓋率（54.83% → 目標 70%+）

| 優先級 | 模組 | 現況 | 建議 |
|--------|------|------|------|
| 高 | `AdminConsolePage.jsx` UI | 12.93% | 加入表單提交、草稿管理、抽籤操作測試 |
| 中 | `UserProfile.jsx` UI | 46.50% | 加入 Tab 互動、票券棄權、QR Modal 測試 |
| 中 | `ApiExplorerPage.jsx` | 45.79% | 加入一鍵檢查、進階 API 檢查流程測試 |
| 中 | `NotificationContext.jsx` | 66.83% | 補 WebSocket 訊息處理、標記已讀測試 |
| 低 | `AnimatedThemeToggler.jsx` | 25.85% | 加入主題切換互動測試 |
| ✅ 已完成 | EventsList / EventDetail / AuthContext / Login / Header | 64–100% | — |
...
1. **Quality Gate Passed** — 所有 Sonar way 品質條件均通過
2. **83 個單元測試全部通過** — 涵蓋 API、認證、活動、驗票、通知、路由守衛等核心邏輯
3. **整體覆蓋率 54.83%** — 較先前 27.3% 提升約 **2 倍**
4. **零漏洞、零 Bug、零重複程式碼**
